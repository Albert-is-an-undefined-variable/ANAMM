...
if __name__ == "__main__":
	...
	(ref_structure, ref_model_useless) = process_file(files[0], arguments.indir, arguments.verbose) #Function returns model but we are only interested in the structure

	if arguments.verbose:
		print("The initial complex has %d chains and are the following:" % (ref_structure[0].__len__()))
	...
	if arguments.verbose:
		print("Output files %s saved in %s" % ("macrocomplex.pdb", os.path.abspath(arguments.outdir)))
